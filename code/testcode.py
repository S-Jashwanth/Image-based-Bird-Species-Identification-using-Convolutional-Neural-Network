...
model_path = 'C:\\Users\\DATA_POINT_INFO\\OneDrive\\Desktop\\bird\Bird-Classification-main\\Bird-Classification-main\\bird_classification\\trained-models\\bird-resnet34best.pth'  # Replace with the actual path to your model
...
def classify_bird(request):
    ...
    if request.method == 'POST' and request.FILES['image']:
        ...
        # Define the path to save the processed image in the 'output' folder
        ...
        # Perform bird classification
        with torch.no_grad():
            output = model(image)
            probabilities = torch.softmax(output, dim=1)
            probability, predicted_class = torch.max(probabilities, dim=1)

        print('predicted_class : ',predicted_class)
        # Get the bird name from the map
        bird_name = bird_name_map.get(predicted_class.item(), 'Unknown')
        probability = round(probability.item() * 100, 2)

        # Create a dictionary with the classification result
        result = {'bird_name': bird_name, 'probability': probability, 'image_url': uploaded_image_url}

    return render(request, 'users/classify.html', {'result': result})
