...
def ClassifyBird(request):
    ...
    if request.method == 'POST':
        ...
        # Use FileSystemStorage to save the uploaded image
        fs = FileSystemStorage(location="media/output/")
        filename = fs.save(image_file.name, image_file)
        image_path = os.path.join(settings.MEDIA_ROOT, 'output', filename)
        ...
        model_path = os.path.join(settings.MEDIA_ROOT, 'trained-models', 'bird-resnet34best.pth')
        ...
        bird = bird_name_map.get(preds[0].item())
        probability = str(round(torch.max(yb).item(), 4) * 100) + '%'
        print('++++++++++++++++++++++++++++++++++++++++++++++++')

        # Save the image generated by Matplotlib to a file
        output_image_path = os.path.join(settings.MEDIA_ROOT, 'output', 'output_image.png')
        print('++++++++++++++++++++++++++++++++++++++++++++++++')
        ...
        try:
            ...
            return HttpResponseServerError(f"Error displaying image: {str(e)}")
        # plt.imshow(im)
        # plt.savefig(output_image_path)
        ...
        print('++++++++++++++++++++++++++++++++++++++++++++++++')
        data={
            'bird': bird, 'probability': probability, 'output_image_path': output_image_path

        }

        # Pass the output image path to the template
        return render(request, 'users/classify.html', data)

    else:
        return render(request, 'users/classify.html', {})
...
def train_model(request):
    if request.method == 'POST':
        # Define the paths to your training, test, and validation data
        train = os.path.join(settings.MEDIA_ROOT, 'train')
        test = os.path.join(settings.MEDIA_ROOT, 'test')
        valid = os.path.join(settings.MEDIA_ROOT, 'valid')
        ...
        valid_image = valid_generator.flow_from_dataframe(dataframe=df_valid,
                                                          x_col='filepaths',
                                                          y_col='labels',
                                                          subset='training',
                                                          target_size=(224, 224),
        ...
        # Plot and display training progress
        history_df = pd.DataFrame(mo_fit.history)
        ...
        # Return the paths to the saved plots along with the rendered template
        context = {
            'accuracy_plot': accuracy_plot_path,
            'loss_plot': loss_plot_path,
        }
        
        return render(request, 'users/train_model.html', context)
    else:
        return render(request, 'users/train_model.html')
